...
def investment_table(connection, positions: pd.DataFrame) -> pd.DataFrame:
    ...
    investments["reference_price_eur"] = reference.astype(float).round(2)
    investments["expected_upside_pct"] = (
    ...
    investments["expected_profit_eur"] = (
        (investments["expected_price_eur"] - reference.fillna(0)) * quantity
    ).astype(float).round(2)

    upside = (investments["bull_price"].fillna(investments["target_price"]) - reference).astype(float)
    downside = (reference - investments["downside_price"].fillna(0)).astype(float)
